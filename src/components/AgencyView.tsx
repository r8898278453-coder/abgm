...
export const AgencyView: React.FC<AgencyViewProps> = ({ business: _business, onNavigate = (_tab: any) => {} }) => {
  const [activeTab, setActiveTab] = useState<'branches' | 'whitelabel' | 'reports'>('branches');
  const [customDomain, setCustomDomain] = useState('portal.apexmarketing.agency');
      ...
      {activeTab === 'branches' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            >
              <Plus className="w-4 h-4" /> Add Branch / Client
            </button>
          </div>
          ...
          <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-start gap-3 text-xs text-indigo-900">
            ...
            <div>
              ...
              <div className="mt-1 text-indigo-800 leading-relaxed font-mono text-[11px]">
                CNAME target: cname.localpulse.ai | Status: SSL Provisioned & Active
              </div>
            </div>
          </div>
        </div>
